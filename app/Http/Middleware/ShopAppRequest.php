...
class ShopAppRequest
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        $isVerified = $this->verifyHmac($request);
        if(!$isVerified) {
            abort(401);
        }
    ...
    private function getHmac(Request $request): ?array
    {
        ...
        $options = [
            ...
            // Headers
            DataSource::HEADER()->toNative() => function () use ($request) {
                $authParams = json_decode($request->header('shopify'), true);
                if(isset($authParams['hmac'])) {
                    return $authParams['hmac'];
                }
                return $request->header('X-Shop-Signature');
            },
            // Headers: Referer
    ...
    private function getData(Request $request, string $source): array
    {
        ...
        $options = [
            ...
            DataSource::HEADER()->toNative() => function () use ($request): array {
                $verify = $this->hasCustomHeader($request);
                if (!$verify) {
                    // Always present
    ...
    protected function hasCustomHeader($request) {
        if($request->header('shopify')) {
            return json_decode($request->header('shopify'), true);
        }
        return null;
    }
}
